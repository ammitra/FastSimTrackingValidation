...
def BtagValPlotting(valdirs,names):
    ...
    first = valdirs[0] if '.root' in valdirs[0] else valdirs[0]+'/FastSimValWorkspace/BTAGVAL/harvest_%s.root'%names[0]
    second = valdirs[1] if '.root' in valdirs[1] else valdirs[1]+'/FastSimValWorkspace/BTAGVAL/harvest_%s.root'%names[1]
